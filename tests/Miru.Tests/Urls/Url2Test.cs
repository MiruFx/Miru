...
{
    [Ignore("WIP")]
    public class Url2Test : IDisposable
    {
        ...
        private IUrlMaps UrlMaps => _host.Services.GetService<IUrlMaps>();
        ...
        {
            var request = new ProductsList.Query { Category = "Jeans", MaxPrice = 70 };

            var url = new UrlBuilder2<ProductsList.Query>(request, new UrlOptions(), UrlMaps).ToString();
            
            url.ShouldBe("/Products/List/Jeans?MaxPrice=70");
        }
