...
namespace MiruCli
{
    public class AtAppCommand : Command
    {
        private readonly MiruSolution _miruSolution;

        public AtAppCommand(string commandName, MiruSolution miruSolution) : base(commandName)
        {
        ...
        private void Execute(string executable, string[] args)
        {
            ...
            var proc = processRunner.RunAsync(new ProcessSpec()
            {
                ...
                Arguments = args,
                WorkingDirectory = _miruSolution.AppDir
            });
