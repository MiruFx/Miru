...
namespace MiruCli
{
    public class AtTestCommand : Command
    {
        private readonly MiruSolution _miruSolution;

        public AtTestCommand(string commandName, MiruSolution miruSolution) : base(commandName)
        {
        ...
        private void Execute(string executable, string[] args)
        {
            ...
            var proc = processRunner.RunAsync(new ProcessSpec()
            {
                ...
                Arguments = args,
                WorkingDirectory = _miruSolution.TestsDir
            });
